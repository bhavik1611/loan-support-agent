...
Amended again 2026-09-13 during implementation, off one grilling round of that date, after execution contradicted four things the design had asserted.
D-78 and D-84 carry corrections to their own rationales, and section 16's test 39 is reworded with them; D-53 is amended; D-86 is new.
Every one of the four was found by running the library rather than reading it, and each records what was measured beside what it replaced.
An amendment that corrects a rationale is marked in place rather than rewritten, so a reader sees what was believed and what measurement said instead.
...
| D-78 | How deep async goes, and what the two timeouts are | `lookup_status` becomes `async def` and carries the library's own `timeout`. `ask()` keeps its signature and wraps `ainvoke` in `asyncio.run`. The global timeout is `asyncio.wait_for` around `ainvoke`. `step_timeout` is unused | **This supersedes the round-1 answer**, which was a native per-node timeout on a sync node plus a wall-clock guard, and which is not implementable. **Measured** on langgraph 1.2.11: a sync node carrying `timeout` raises `ValueError`, "Node timeouts are only supported for async nodes because sync Python execution cannot be safely cancelled in-process", and so does an async node driven by `invoke`; under `ainvoke` the same node raised `NodeTimeoutError` cleanly at 0.101s against a 0.100s budget. Making `ask()` itself async lost because it rewrites 19 call sites and needs `pytest-asyncio`, a new dependency, for what the sync wrapper gets without one. Hand-rolling both timeouts lost because it implements the exact thing the library refuses as unsafe, and criterion 16b asks for a clean error rather than a hang. `step_timeout` bounds one superstep rather than the run, so calling it the global timeout would be 18.4's pattern again. **Measured blast radius**: transcripts and README byte-identical, 370 of 372 tests untouched, the two failures being direct sync calls at `tests/test_nodes.py:131` and `:139`. **Amended 2026-09-13 during implementation: this rationale was true and insufficient on two counts, both found by measurement.** First, `async def` alone buys only the **build-time** check. A coroutine that never awaits cannot be preempted by any watchdog, so the timeout was accepted and could never fire: measured, an async node with `timeout=0.1` whose body calls `time.sleep(1.0)` returned after 1.014s with no timeout, while the same node wrapping the call in `await asyncio.to_thread(...)` raised `NodeTimeoutError` at 0.106s. `lookup_status` therefore wraps its tool call in `asyncio.to_thread`; `agent/tools.py` stays synchronous and the wrapper lives at the call site. Second, **both timeouts bound the graph's execution, not `ask()`'s wall clock.** `ask()` wraps `asyncio.run`, which shuts its loop down via `shutdown_default_executor()` and waits for every worker thread it handed work to; CPython cannot kill a running thread, so an abandoned tool call runs to completion first. Measured: the graph cancelled on budget at 0.203s while `asyncio.run` did not return for a further 2.810s. Carried rather than fixed (a private executor torn down without waiting, or a process pool, is real complexity bought for a local SQLite read of single-digit milliseconds), and documented on `ask()` itself. |
...
| D-84 | Whether any triad number is pinned by a test | Nothing numeric. One test asserts the ordering property: the two `far_out_of_scope` rows score below every one of the twelve answerable rows on groundedness | Pinning the three averages lost for D-13's reason, that a test which fights tuning gets deleted, and the brief only asks for these numbers to be reported. Asserting nothing lost because the scorers could silently invert and every transcript would still read plausibly. **Amended 2026-09-13 during implementation, and the original rationale was measured false.** It read "a judge that cannot mark IU-02 down is broken regardless of what its averages read", and it covered all three non-answerable rows. IU-02 is not refused: it is answered at top-1 0.4645 against T=0.3066 with all three chunks parented to `kb-12-nri-account-eligibility`, so the support rule passes too. Its answer faithfully restates that irrelevant context, so groundedness reads 0.7857, above seven of the twelve answerable rows and below only five, and that is groundedness reporting **correctly**. An answer grounded in the wrong document is grounded by definition; catching it is context relevance's job, which is the whole argument for three signals rather than one. Retuning the scorers until the classes separate was rejected as fitted to one item, because IU-02's context relevance (0.2000) already reads higher than correctly-answered EQ-12's (0.1667), so no cut on either signal separates them. Retuning retrieval so IU-02 refuses was rejected because T would have to exceed 0.4645, which also refuses EQ-07, EQ-11 and EQ-12 and regenerates every Part 1 number. IU-02 is now pinned as its own documented limitation instead, relationally against the weakest answerable row. |
...
| D-86 | What retrieval's product filter does about a document uploaded at runtime | The `doc_id` `$in` clause is the catalogue's ids **unioned with** the uploaded ids, derived deterministically from `config.UPLOAD_DIR`. `knowledge_base/` and `catalogue.json` stay untouched | **Found during implementation, not at design time, and it made a graded criterion unreachable.** D-72 keeps uploads out of `knowledge_base/`; D-53 narrows retrieval to the doc ids `catalogue.json` tags. An upload is in neither, so no query naming a catalogue product could ever retrieve one, and the upload feature was inert for exactly the queries a user is most likely to ask. **Measured**: the plan's own demo query, "How long does a home loan take to disburse after approval?", scored the uploaded document at 0.8006 unfiltered while the product-narrowed path never saw it, leaving before and after bit-identical at 0.4534. Documenting it as a V1 boundary and demoing with a query that names no product was recommended and **overruled by Bhavik**, who chose to fix the filter. He was right: the demo now uses the original product-naming query and goes from refused at top-1 0.4534 to answered at 0.8702 citing `kb-up-01`, which is a stronger demonstration than one that quietly avoids the broken case. Writing uploads into `catalogue.json` lost because it reopens everything D-72 closed. Teaching chunk metadata about products lost for the reason already recorded in D-53. **The load-bearing property, verified rather than assumed**: with no uploads present the union is byte-identical to the old clause, so `run_part1.py` and `run_part2.py` regenerate every transcript and README number unchanged, `git diff` returning zero lines. **That property is now enforced, not trusted.** Because the filter reads `data/uploads/` live, a stray upload would silently move graded artefacts with no code change and no error, so `run_part1.py`, `run_part2.py`, `run_part4.py` and `tests/conftest.py` each refuse to run while that directory holds a file. They refuse rather than delete, matching `require_mock_provider`: the files are the user's. |
...
The `products` list is read out of the documents, never invented.
...
| 39 | Every `far_out_of_scope` triad row scores below every answerable row on groundedness | D-84; the property, never the numbers. Amended 2026-09-13: `inside_uncovered` is excluded because IU-02 is answered rather than refused, and a grounded restatement of the wrong document is correctly grounded. It is pinned separately as a limitation |
