...
# The markers README.md carries around each generated table. They are a matched
# pair so that "inside the block" is defined for a checker; an opening marker
# with no closing one leaves the end of the generated region to guesswork.
#
# Neither marker carries a date. It used to stamp date.today(), which made the
# generated bytes a function of the calendar rather than of the seed: a rerun
# on any later day dirtied the working tree with no measured number having
# moved, on the one repository whose headline claim is a byte-identical rerun.
# A reader who learns to ignore a dirty tree has lost the only signal that
# claim has. The date a block was produced is in git, which is where it belongs.
BLOCK_OPEN = "<!-- Generated by scripts/run_part1.py. Do not edit by hand. -->"
BLOCK_CLOSE = "<!-- End of generated block. -->"


def block(heading: str, body: list[str]) -> list[str]:
    """One README section, wrapped in the matched markers README.md carries."""
    return [f"### {heading}", "", BLOCK_OPEN, ""] + body + ["", BLOCK_CLOSE, ""]
...
def generation_transcript() -> str:
    ...
    for strategy in sorted(config.COLLECTION_FOR_STRATEGY):
        ...
        for query in EVAL_QUERIES:
            lines += _one_answer(generate.answer(query.text, strategy), query.item_id)
...
def readme_numbers(summary: dict, counts: dict) -> str:
    """Generate the README numbers, one marked block per README section."""
    ...
    lines = (
        ...
        + block("Chunking comparison", chunking_rows)
    )
    return "\n".join(lines).rstrip("\n") + "\n"
